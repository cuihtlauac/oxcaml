...
let rec denv_of_decision denv ~param_var (decision : U.decision) : DE.t =
  ...
  | Unbox (Number (Naked_vec256, { param = naked_vec256; args = _ })) ->
    let shape =
      T.boxed_vec128_alias_to ~naked_vec128:naked_vec256 (Alloc_mode.For_types.unknown ())
    in
  ...
  | Unbox (Number (Naked_vec512, { param = naked_vec512; args = _ })) ->
    let shape =
      T.boxed_vec128_alias_to ~naked_vec128:naked_vec512 (Alloc_mode.For_types.unknown ())
    in
