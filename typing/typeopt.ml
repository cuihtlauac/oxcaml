...
and sort_to_scannable_product_element_kind loc (s : Jkind.Sort.Const.t) =
  ...
  | Base (Float64 | Float32 | Bits32 | Bits64 | Word | Vec128 | Vec256 | Vec512) as c ->
...
and sort_to_ignorable_product_element_kind loc (s : Jkind.Sort.Const.t) =
  ...
  | Base (Vec128 | Vec256 | Vec512) -> raise (Error (loc, Unsupported_vector_in_product_array))
...
let value_kind_of_value_jkind env jkind =
  ...
  | Any, _
  | Product _, _
  | Base (Void | Float64 | Float32 | Word | Bits32 | Bits64 | Vec128 | Vec256 | Vec512) , _ ->
